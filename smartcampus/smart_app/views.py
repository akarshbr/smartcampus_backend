...
class Enqui_studentAPIView(APIView):
    authentication_classes = [JWTAuthentication]
    permission_classes = [IsAuthenticated]
    def get(self, request, format=None):
        enquiries = Enqui_student.objects.all()
        serializer = Enqui_studentSerializer(enquiries, many=True)
        return Response({'status':1,'data':serializer.data})
    ...
    def post(self, request, format=None):
        ...
        # # Create the enquiry object with the user
        # enquiry_data = {'user_name': user.id}  # Assuming user_name is the ForeignKey to CustomUser
        # enquiry_data.update(request.data)
        # print("enqu",enquiry_data)
        serializer = Enqui_studentSerializer(data= request.data)
        print("ser",serializer)
        if serializer.is_valid():
            serializer.save(user_name = request.user)
            return Response({'status':1,'data':serializer.data,'message':"enquiry added successfully"}, status=status.HTTP_201_CREATED)
